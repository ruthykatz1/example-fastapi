...
@pytest.mark.parametrize("title, content, published", [
    ("Awsome new post", "Awsome content", True),
    ("I like Pizza", "Extra cheese", False),
    ("I like steaks", "My favorite", True)
])
# test_posts isn't mandatory
...
def test_create_post_default_published_true(authorized_client, test_user, test_posts):
    res = authorized_client.post("/posts/", json={"title":"random_t", "content":"random_c"})
    created_post = schemas.Post(**res.json())
    assert res.status_code == 201
    assert created_post.title == "random_t"
    assert created_post.content == "random_c"
    assert created_post.published == True
    assert created_post.owner_id == test_user['id']

def test_unauthorized_user_create_post(client):
    res = client.post("/posts/", {"title":"adad", "content":"rtrgg"})
    assert res.status_code == 401
...
def test_update_post_non_exist(authorized_client):
    data = {"title": "Updated title", "content": "Updated content", "id": 800000000}
    res = authorized_client.put(f"/posts/800000000", json=data)
    assert res.status_code == 404